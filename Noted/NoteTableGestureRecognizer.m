...
typedef enum {
    NoteTableGestureRecognizerStateNone,
    NoteTableGestureRecognizerStateDragging,
    NoteTableGestureRecognizerStatePinching,
    NoteTableGestureRecognizerStatePanning,
    NoteTableGestureRecognizerStateMoving,
} NoteTableGestureRecognizerState;

CGFloat const NoteTableCommitEditingRowDefaultLength = 80;
CGFloat const NoteTableRowAnimationDuration          = 0.25; 

@interface NoteTableGestureRecognizer () <UIGestureRecognizerDelegate>
@property (nonatomic, assign) id <NoteTableGestureAddingRowDelegate, NoteTableGestureEditingRowDelegate, NoteTableGestureMoveRowDelegate> delegate;
...
@property (nonatomic, assign) NoteCellEditingState  addingCellState;
...
@property (nonatomic, assign) NoteTableGestureRecognizerState state;
...
@implementation NoteTableGestureRecognizer
...
- (void)commitOrDiscardCell {
    ...
    [self.tableView performSelector:@selector(reloadVisibleRowsExceptIndexPath:) withObject:self.addingIndexPath afterDelay:NoteTableRowAnimationDuration];
    ...
    self.state = NoteTableGestureRecognizerStateNone;
}
...
- (void)pinchGestureRecognizer:(UIPinchGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        self.state = NoteTableGestureRecognizerStatePinching;
...
- (void)panGestureRecognizer:(UIPanGestureRecognizer *)recognizer {
    if ((recognizer.state == UIGestureRecognizerStateBegan
        ...
        self.state = NoteTableGestureRecognizerStatePanning;
        ...
        CGFloat commitEditingLength = NoteTableCommitEditingRowDefaultLength;
        ...
        if (fabsf(translation.x) >= commitEditingLength) {
            if (self.addingCellState == NoteCellEditingStateMiddle) {
                self.addingCellState = translation.x > 0 ? NoteCellEditingStateRight : NoteCellEditingStateLeft;
            }
        } else {
            if (self.addingCellState != NoteCellEditingStateMiddle) {
                self.addingCellState = NoteCellEditingStateMiddle;
            }
        }
...
- (void)longPressGestureRecognizer:(UILongPressGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        self.state = NoteTableGestureRecognizerStateMoving;
        ...
        __block NoteTableGestureRecognizer *weakSelf = self;
        ...
        [UIView animateWithDuration:NoteTableRowAnimationDuration
                         animations:^{
                             ...
                             weakSelf.state = NoteTableGestureRecognizerStateNone;
                         }];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == self.panRecognizer) {
        if ( ! [self.delegate conformsToProtocol:@protocol(NoteTableGestureEditingRowDelegate)]) {
            return NO;
        }
        ...
        if ( ! [self.delegate conformsToProtocol:@protocol(NoteTableGestureAddingRowDelegate)]) {
        ...
        if (indexPath && [self.delegate conformsToProtocol:@protocol(NoteTableGestureMoveRowDelegate)]) {
...
- (CGFloat)tableView:(UITableView *)aTableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([indexPath isEqual:self.addingIndexPath]
        && (self.state == NoteTableGestureRecognizerStatePinching || self.state == NoteTableGestureRecognizerStateDragging)) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.y < 0) {
        ...
        if ( ! self.addingIndexPath && self.state == NoteTableGestureRecognizerStateNone && ! scrollView.isDecelerating) {
            self.state = NoteTableGestureRecognizerStateDragging;
    ...
    if (self.state == NoteTableGestureRecognizerStateDragging) {
...
+ (NoteTableGestureRecognizer *)gestureRecognizerWithTableView:(UITableView *)tableView delegate:(id)delegate {
    NoteTableGestureRecognizer *recognizer = [[NoteTableGestureRecognizer alloc] init];
...
- (NoteTableGestureRecognizer *)enableGestureTableViewWithDelegate:(id)delegate {
    if ( ! [delegate conformsToProtocol:@protocol(NoteTableGestureAddingRowDelegate)]
        && ! [delegate conformsToProtocol:@protocol(NoteTableGestureEditingRowDelegate)]
        && ! [delegate conformsToProtocol:@protocol(NoteTableGestureMoveRowDelegate)]) {
        [NSException raise:@"delegate should at least conform to one of NoteTableGestureAddingRowDelegate, NoteTableGestureEditingRowDelegate or NoteTableGestureMoveRowDelegate" format:nil];
    }
    NoteTableGestureRecognizer *recognizer = [NoteTableGestureRecognizer gestureRecognizerWithTableView:self delegate:delegate];
    return recognizer;
}
